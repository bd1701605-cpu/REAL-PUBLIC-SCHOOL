...
const AttendanceTracker = ({ role, classIds, studentId }: { role: UserRole, classIds: string[], studentId: string }) => {
  ...
  if (role === UserRole.TEACHER || role === UserRole.ADMIN) {
    const allClasses = dbService.getClasses();
    const dropdownClasses = role === UserRole.ADMIN ? allClasses.map(c => c.id) : classIds;
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <select 
              ...
              className="border-2 border-slate-100 rounded-2xl px-6 py-3 text-sm font-black outline-none focus:border-blue-600 bg-white shadow-sm"
            >
              <option value="">Select Unit...</option>
              {dropdownClasses.map(cid => (
        ...
        <Card title={`Registry for Unit: ${selectedClassId || 'Unselected'}`}>
          <div className="space-y-3">
            {students.length === 0 ? (
              <div className="text-center py-20 border-2 border-dashed rounded-3xl text-slate-300 font-bold italic">No students found in this unit.</div>
            ) : (
              students.map(s => {
                const todayStatus = getTodayStatus(s.id);
                return (
                  <div key={s.id} className="flex items-center justify-between p-5 border-2 border-slate-50 rounded-3xl bg-white hover:border-blue-100 transition-all shadow-sm">
                    <div className="flex items-center space-x-4">
                      <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(s.name)}&background=random&bold=true`} className="w-12 h-12 rounded-2xl shadow-sm" />
                      <div>
                        <p className="text-md font-black text-slate-800">{s.name}</p>
                        <p className="text-[10px] text-slate-400 font-mono uppercase font-black">{s.uid}</p>
                      </div>
                    </div>
          ...
          <div className="fixed inset-0 bg-[#0f172a]/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
             <div className="bg-white rounded-[40px] p-10 w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-6">
                   ...
                   <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Instructional Body</label>
